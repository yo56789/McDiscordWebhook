	...
	public void onInitializeServer() {
		...
		if (Config.EVENT_PLAYER_JOIN_ENABLED) {
			ServerPlayConnectionEvents.JOIN.register((ServerPlayNetworkHandler handler, PacketSender sender, MinecraftServer server) -> {
				String data = WebhookHandler.assembleMessage(String.format(Config.EVENT_PLAYER_JOIN, handler.getPlayer().getName().getString()), Config.SERVER_NAME, Colors.GREEN.colorCode);
				WebhookHandler.post(Config.WEBHOOK_URI, data);
			});
		}
		...
		if (Config.EVENT_PLAYER_LEAVE_ENABLED) {
			ServerPlayConnectionEvents.DISCONNECT.register((ServerPlayNetworkHandler handler, MinecraftServer server) -> {
				String data = WebhookHandler.assembleMessage(String.format(Config.EVENT_PLAYER_LEAVE, handler.getPlayer().getName().getString()), Config.SERVER_NAME, Colors.RED.colorCode);
				WebhookHandler.post(Config.WEBHOOK_URI, data);
			});
		}
